...
def findGenres (movieTitle):
    ...
    result = subprocess.run(["g++", "MovieListParser.cpp"], capture_output=False)
    ...
    return (result.stdout) # prints the output of the execution

@app.route("/ParseList/<genre>")
def GetList(genre):
    result = subprocess.run(["g++", "MovieListParser.cpp"], capture_output=True)
    #print(result.stdout) # prints the output of the compilation
    genre = genre.encode()
